...
type WeddingExperienceProps = {
  /** Si se entrega, reemplaza la sección de RSVP por una versión personalizada. */
  rsvpSlot?: ReactNode;
};

export function WeddingExperience({ rsvpSlot }: WeddingExperienceProps) {
  return (
    <>
      <ScrollProgress />
      <PetalsCanvas />
      ...
      {/* Fixed Controls */}
      <div className="safe-top fixed right-4 top-4 z-50 flex items-center gap-2 md:right-8 md:top-8">
      ...
      <main>
                  ...
                  <div className={`hidden md:block ${i % 2 === 0 ? "order-1" : "order-3"}`}>
                    {i % 2 === 1 && (
                      <div className="timeline-card ml-auto max-w-sm">
                        <p className="mb-1 text-xs uppercase tracking-[0.3em] text-gold">0{i + 1}</p>
        ...
        <footer className="section-shell safe-bottom pb-16">
          ...
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:justify-between">
            <p className="text-[9px] uppercase tracking-[0.3em] text-muted">{weddingConfig.footer.text}</p>
